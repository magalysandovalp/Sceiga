...
void loop() {
  ...
  if (!gsmModule.isRegistered()) {
    ...
    if (gsmModule.isRegistered()){
      ...
      delay(2000);
      gsmModule.sendSMS("83237670",mensajeTwitter);      
      //char mensajeTwitter[140] = "y9zw5z-g0m4y0@twittermail.com Temperatura: ";
    }
    delay(3000);
  }
...
void nuevoArchivo(){                     //Función encargada de crear un archivo nuevo para escribir los valores del juego de mediciones actuales 
  ...
  delay(500); 
  Serial1.print("new Rec" + String(randomaleat) +  ".csv");    //Crea el archivo llamado Rec + el numero random leido en el pin analógico + .csv esto con el fin de poder abrirlo en excel
  delay(500);  
  Serial1.println("");
  delay(500);  
